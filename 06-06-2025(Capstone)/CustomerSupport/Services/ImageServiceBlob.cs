...
public class ImageServiceBlob : IImageService
{
    ...
    public ImageServiceBlob(IRepository<string, Image> imageRepository,
    ...
    public async Task<ImageReponseDto> DownloadImage(string userId, int chatId, string imageName)
    {
        ...
        var isUserInChat = await _otherContextFUnctions.IsUserInChat(chatId, userId);
        if (!isUserInChat)
            throw new UnauthorizedAccessException("User not authorized to download image from this chat");

        var blobClient = _containerClient.GetBlobClient(imageName);

        if (!await blobClient.ExistsAsync())
            throw new FileNotFoundException($"File with name: {imageName} not found");
    ...
    public async Task<string> UploadImage(string userId, int chatId, ImageUploadDto imageDto)
    {
        ...
        var blobClient = _containerClient.GetBlobClient(image.ImageName);
        await blobClient.UploadAsync(imageDto.formFile.OpenReadStream(), overwrite: true);

        await CreateImageMessage(chatId, image.ImageName, userId);
